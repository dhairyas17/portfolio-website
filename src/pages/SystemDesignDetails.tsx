...
import { systemDesignCases } from '../data/systemDesigncases';



const SystemDesignDetails = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const filteredList = systemDesignCases;
  ...
  const sp = systemDesignCases.find((p) => p.id === id);
  ...
  if (!sp) {
    return (
      <div className="min-h-screen pt-20 pb-16 text-center text-gray-600">
        ...
        <Link
          to="/system-product-design/system-design"
    ...
    >
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div className="mb-8" initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.2 }}>
          <Link to="/system-product-design/system-design" className="inline-flex items-center text-blue-600 hover:text-blue-800 mb-6 transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <Metric icon={<Clock />} label="Duration" value={sp.duration} />
            <Metric icon={<Users />} label="Team Size" value={sp.team} />
            <Metric icon={<TrendingUp />} label="Impact" value={sp.impact} highlight />
          </div>
        ...
        <motion.div className="mb-12" initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.4 }}>
          <img src={sp.image} alt={sp.title} className="w-full h-64 md:h-96 object-cover rounded-xl shadow-lg" />
        </motion.div>
        ...
        <div className="space-y-12">
          ...
          <Section title="Architecture" delay={0.85}>
            <p className="mb-4">{sp.architecture.description}</p>
            {sp.architecture.diagram && (
              <img src={sp.architecture.diagram} alt="Architecture Diagram" className="rounded-xl shadow-lg" />
            )}
          </Section>

          <Section title="Design Goals" delay={0.9}>
            <ul className="list-disc pl-6 space-y-2">
              {sp.designGoals.map((goal, idx) => (
                <li key={idx}>{goal}</li>
              ))}
            </ul>
          </Section>
          ...
          <Section title="Implementation Timeline" delay={1.0}>
            <ul className="space-y-6">
              {sp.implementation.map((phase, idx) => (
                <li key={idx}>
        ...
        <div className="mt-20 flex flex-col sm:flex-row justify-between gap-4">
          {prevItem ? (
            <button onClick={() => navigate(`/system-product-design/system-design/${prevItem.id}`)} className="group flex items-center gap-3 px-4 py-3 border rounded-lg">
          ...
          {nextItem ? (
            <button onClick={() => navigate(`/system-product-design/system-design/${nextItem.id}`)} className="group flex items-center gap-3 px-4 py-3 border rounded-lg">
...
export default SystemDesignDetails;
